// websocketAudio.ts
// companion to the websocketAudio adapter in the server
...
export class WebsocketAudio {
  ...
  // audio out
  private outAudioContext: AudioContext | null;
  private sourceNode: AudioBufferSourceNode | null;
  private bufferQueue: ArrayBuffer[]; // Queue to store audio buffers
  private isPlaying: boolean; // Flag to check if audio is playing
  // audio in
  private inAudioContext: AudioContext | null;
  ...
  async start(): Promise<void> {
    try {
      ...
      this.socket.onopen = () => {
        ...
        const sessionStarted: StartSessionMessage = {
          event: 'start',
          start: {
  ...
  queuePcmData(pcmData: ArrayBuffer): void {
  ...
  async playFromQueue(): Promise<void> {
    ...
    this.isPlaying = true;
    const pcmData = this.bufferQueue.shift() as ArrayBuffer; // Get the next chunk from the queue
    ...
    const source =
      this.outAudioContext?.createBufferSource() as AudioBufferSourceNode;
    source.buffer = audioBuffer;
    source.connect(this.outAudioContext?.destination as AudioNode);
    source.onended = () => {
  ...
  async decodePcm16Data(pcmData: ArrayBuffer): Promise<AudioBuffer> {
  ...
  extractPcm16Data(buffer: AudioBuffer): ArrayBuffer {
